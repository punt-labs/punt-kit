...
| [Repo-Local State Directory](standards/punt-labs-dir.md) | `<repo>/.punt-labs/<tool>/` as the sole repo-local root; committed except local-convention paths (`local/`, `*.local*`); live state never tracked (seal pattern); canonical gitignore block; subtree zones |
