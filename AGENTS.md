...
| [Repo-Local State Directory](standards/punt-labs-dir.md) | `<repo>/.punt-labs/<tool>/` as the sole repo-local root, committed-except-`*local*`, live state never tracked (the seal pattern), the canonical gitignore block, subtree zones (vendored/config/local/marker) |
