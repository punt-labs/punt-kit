...
- **Phase 5's `--resume-from tag` now re-pushes a tag that was created locally but never reached the remote.** `Phase5Tag.run` creates the git tag locally before pushing it; a push failure (network blip, transient auth failure) left the tag at HEAD locally with nothing on the remote. Resuming re-entered the existing-tag branch, saw the tag already at HEAD, logged "already exists," and returned without ever retrying the push — silently sending Phase 6 to wait on CI for a tag that was never pushed. The existing-tag branch now checks `git ls-remote --tags origin` and pushes when the tag is missing from the remote, leaving the genuinely-already-pushed case a no-op (pkit-f85t.10, P0).
