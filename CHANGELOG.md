...
- **`standards/punt-labs-dir.md`, `standards/tool-enable-disable.md`, and `standards/filesystem.md` codify two operator rulings on per-repo config location, 2026-09-10.** Ruling 1: every per-repo tool artifact lives under `.punt-labs/<tool>/` — no repo-root sentinel survives in any form. A fleet survey (30+ repos) found three attested legacy shapes, not the one form the standard previously named: a settings dotfile (`.biff`, `.quarry.toml`), a config-bearing root directory (`.vox/config.md`, `.lux/config.md` — both carry live daemon-mutable settings, not "pure presence" as an earlier draft assumed), and a root-level runtime-state directory (`.ethos/missions.jsonl`, tracked in every surveyed ethos-adopting repo). `punt-labs-dir.md` §1/§8/§9/§10 name and grade all three, and `tool-enable-disable.md`/`filesystem.md` now cross-reference that table instead of restating the dotfile-only list. Ruling 2, on the standing question of what git status a daemon-rewritten file should have inside `.punt-labs/<tool>/`: "*local* is the flag that something should not be in git. If it is not in *local*, that means I designed it to be in git." Git status inside the tool root is naming-determined and naming-determined only (`punt-labs-dir.md` §4) — a daemon-rewritten file like `vox.md` is ordinary tracked Config-zone content, not a separate gitignored zone; it legitimately shows `modified` in an active checkout by design, and the release engine's accommodation for that (stash around the clean-tree check) is tracked separately as `pkit-f85t.9`. `punt-labs-dir.md` §8 gains the audit check this implies: a **gitignored** non-local-convention path under `.punt-labs/<tool>/` is the violation, not a tracked one.
