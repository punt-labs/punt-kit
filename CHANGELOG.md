...
- **`docs/design-release-failure-harness.md` designs the release-engine failure-path test harness.** ACCEPTED design for `pkit-f85t`'s remaining shared deliverable: a `FaultInjectingOps` fault-injection double layered on the existing `ReleaseOps` seam, a recorded-`gh`-fixture contract-test library to catch CLI drift, and rule-anchored SIGINT/blocking-call primitives for reproducing DES-029's mechanism directly — plus a 61-row fault-injection coverage matrix and an 8-item known-defect inventory (including a newly-identified P0 in Phase 5's tag-then-push resume ordering) sequenced into six incremental delivery waves. No source changes in this PR — the design lands first per the org's design-then-implement workflow.
