...
- **`tests/test_release.py` ships pkit-f85t.7's per-site harness scenarios for the nineteen individual `ops.run` call sites the diagnosed-`check=False` sweep converts across two rounds (see Fixed, below).** Total added wall-clock across all twenty-six new tests in this wave (the eleven matrix-row tests plus fifteen pkit-f85t.7 sweep-scenario tests): 1.67s.
