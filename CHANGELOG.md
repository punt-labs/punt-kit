...
- **Phase 5 now tags the actual release commit, not `main` HEAD at tag time.** Phase 4c lands a README-install-SHA-pin commit on `main` right after the squash-merge, so by the time Phase 5 ran, `main` HEAD was already one commit past the release — and `git tag` tagged that later commit, drifting the version tag onto the README bump instead of the release. Phase 4 now returns the squash-merge SHA it already produces and threads it to Phase 5, which tags that commit explicitly; the SHA is canonicalized to full length so the tag-already-exists resume check compares like with like. On `--resume-from tag` with no captured SHA, Phase 5 resolves the release commit from `main`'s history by its `chore: release vX.Y.Z` subject and fails loud rather than tagging HEAD (pkit-z9yq).
