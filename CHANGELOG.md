...
- **`standards/punt-labs-dir.md`, `standards/tool-enable-disable.md`, and `standards/filesystem.md` codify the no-repo-root-sentinel and daemon-mutable-zone rulings.** Operator ruling 2026-09-10: every per-repo tool artifact lives under `.punt-labs/<tool>/` — no repo-root sentinel survives in any form. A fleet survey (30+ repos) found three attested legacy shapes, not the one form the standard previously named: a settings dotfile (`.biff`, `.quarry.toml`), a config-bearing root directory (`.vox/config.md`, `.lux/config.md` — both carry live daemon-mutable settings, not "pure presence" as an earlier draft assumed), and a root-level runtime-state directory (`.ethos/missions.jsonl`, tracked in every surveyed ethos-adopting repo). `punt-labs-dir.md` §1/§8/§9/§10 name and grade all three, and `tool-enable-disable.md`/`filesystem.md` now cross-reference that table instead of restating the dotfile-only list. `punt-labs-dir.md` also gains a fifth subtree zone, **daemon-mutable** (§7): a file a running daemon rewrites continuously (the `vox.md` precedent above) is gitignored in place by a permanent per-file rule — distinct from the local-convention naming scheme (the filename must stay stable) and from the interim-exclude mechanism (the file isn't migrating away). This zone is a leader recommendation pending final operator confirmation — flagged as provisional in the originating PR description, not in the standard text itself.
