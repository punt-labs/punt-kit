...
- **`tests/test_release.py` ships Wave 4 of the release-engine failure-path harness (pkit-f85t): the eleven thin-phase matrix-row tests §5's delivery plan names.** Phases 3, 5, 7, and 8 previously had zero isolated failure-path tests at all; this wave closes that gap directly against the merged `FaultInjectingOps`/`FaultRule` harness, with zero real waits — a `TimeoutExpired` fault stands in for every hang, never a real `time.sleep`. Row 3: a non-zero `git fetch origin` during preflight. Row 6: a genuine hang (not just a failure) at the quality-gate call site, diagnosed end-to-end through `run_release` rather than left to traceback. Row 12: `uv build` failing. Row 13: a rejected plugin-swap commit, then a clean retry from the same starting state landing the commit and reaching merge — the fault-injection counterpart to the pre-existing hand-simulated-disk-state test. Row 20: the tag-push failure moment itself, isolated from Wave 2's resume-focused coverage of the same underlying state. Row 29: `gh release create` failing. Row 31: a genuine `TimeoutExpired` mid-retry-loop in Phase 8's 10-attempt PyPI install, distinct from Phase 11's differently-shaped resolve check. Row 32: a rejected restore commit, same fault-injected-failure-then-clean-retry shape as row 13. Row 52: a CLI-only project (install.sh present, no plugin.json at all) driven through every applicable Phase 11 check, proving the plugin.json/marketplace checks correctly don't run for that shape rather than merely implying it. Row 54: a `TimeoutExpired` at a call site that passes no explicit `timeout=` at all, pinning the diagnosis to the genuine `DEFAULT_RUN=60` fallback rather than a named budget. Row 55: a Go quality-gate failure parallel to the existing Python one. Total added wall-clock across all fourteen new tests in this wave (the eleven matrix rows plus the three pkit-f85t.7 sweep scenarios below): under 2 seconds.
- **`tests/test_release.py` ships pkit-f85t.7's per-site harness scenarios for the six call sites the diagnosed-`check=False` sweep converts (see Fixed, below).**
...
- **Six `ops.run` call sites that defaulted to `check=True` now diagnose failures instead of leaking a raw `CalledProcessError` (pkit-f85t.7).** Design doc defect #3 named `InstallAllPropagator._sync_profile_readme`'s `git log` call as one example of a broader pattern: most call sites already pass `check=False` plus a specific `ops.fail(...)` message, but a few still defaulted to `check=True` with no surrounding diagnosis. For a call inside a synchronous, non-threaded phase (3, 4, 5, 9, and the sibling-validation step of 1), the gap was worse than a lower-quality message — a raw `CalledProcessError` has no handler in `run_release` (which only catches `ReleaseError` and `subprocess.TimeoutExpired`) and would have escaped as a bare traceback instead of the usual "Release incomplete — resume with ..." diagnosis. Converted: `Phase3Build`'s `uv build`; `Phase4ReleasePr`'s plugin-swap script invocation; `GitWorkspace.push` (Phase 5's tag push, its only caller); `Phase9PostRelease`'s restore-script invocation and its own explicit restore commit; `SiblingRepo.validate`'s `git branch --show-current` and `git status --porcelain` (inconsistent with the `git pull --ff-only` two lines later in the same method, which was already diagnosed); and `InstallAllPropagator._sync_profile_readme`'s `git log` (defect #3's named example) — distinguished from the already-handled, genuinely-empty "no commit has ever touched install-all.sh" case. Every converted site carries its own harness scenario (see Added, above).
