...
- **Phase 5's `--resume-from tag` now re-pushes a tag that was created locally but never reached the remote — and refuses to proceed if the remote tag is a different commit.** `Phase5Tag.run` creates the git tag locally before pushing it; a push failure (network blip, transient auth failure) left the tag at HEAD locally with nothing on the remote. Resuming re-entered the existing-tag branch, saw the tag already at HEAD, logged "already exists," and returned without ever retrying the push — silently sending Phase 6 to wait on CI for a tag that was never pushed. The existing-tag branch now checks `git ls-remote --tags origin` and pushes when the tag is missing from the remote, leaving the genuinely-already-pushed case a no-op. A remote ref of the same name is not proof it's the *right* commit either — an operator recovering from a botched release by recreating the local tag at a corrected commit, with a stale wrong-commit tag still on the remote, now gets a loud diagnosed failure instead of a silent "already exists" (pkit-f85t.10, P0).
- **`PrMerger.merge_in_sibling`'s secondary cleanup failure now reaches the end-of-run recap, and a hung cleanup checkout no longer masks the primary failure.** If a sibling PR merge fails and the `finally` block's own "return sibling to main" checkout also fails, the secondary failure used to degrade to an info log invisible among Phase 10's concurrent output. `PrMerger` now accepts an optional `skips: SkipRecorder` (threaded through `_sibling_pr_merge`, not `_pr_merge`'s own release-PR merge) and routes this specific case through it, so an operator sees both the primary and secondary failure in "Manual action required". Also fixes a masking hazard flagged in review: the cleanup checkout's `check=False` suppresses a non-zero exit but not a `TimeoutExpired` (a hung git hook) — an uncaught timeout there previously replaced whatever primary exception was already propagating. A timeout during cleanup is now folded into the same secondary-failure reporting when a primary exception exists, or propagates exactly as it always did when the cleanup timeout is the only failure (pkit-f85t.6).
