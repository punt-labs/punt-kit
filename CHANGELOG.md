...
- **`.punt-labs/vox/vox.md` is tracked again.** Operator ruling (2026-09-10): in `.punt-labs/`, the `local` naming convention (`*.local.md`, `local/`) is the ONLY not-in-git flag — everything else is designed to be tracked. #350 untracked `vox.md` and gitignored it as "daemon-mutable config"; that was the divergence — vox and lux already had this right. The `.gitignore` entry #350 added is removed and the file is committed; the `vox.local.md` and `ephemeral/` entries stay.
