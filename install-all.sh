...
banner "Marketplace"
curl -fsSL "$GH/claude-plugins/9c621aa/install.sh" | sh

# --- Step 2: CLI tools ---
# Each installer handles its own pre-flight (Python, uv, SSH fallback).

banner "punt-kit"
curl -fsSL "$GH/punt-kit/58ee42b/install.sh" | sh

banner "biff"
curl -fsSL "$GH/biff/4a86d68/install.sh" | sh

banner "quarry"
curl -fsSL "$GH/quarry/eb7edcf/install.sh" | sh

banner "vox"
curl -fsSL "$GH/vox/736cee4/install.sh" | sh

banner "lux"
curl -fsSL "$GH/lux/8aab4ad/install.sh" | sh
